:- module(
  optparse,
  [
    opt_parse/4, % +OptionSpecifications:list(list(nvpair))
                 % +CommandLineArguments:atom
                 % -Options:list(nvpair)
                 % -PositionalArguments:list(nvpair)
    opt_parse/5, % +OptionSpecifications:list(list(nvpair))
                 % +CommandLineArguments:atom
                 % -Options:list(nvpair)
                 % -PositionalArguments:list(nvpair)
                 % +ParseOptions:list(nvpair)
    opt_arguments/3, % +OptionSpecifications:list(list(nvpair))
                     % -Options:list(nvpair)
                     % -PositionalArguments:list(nvpair)
    opt_arguments/4, % +OptionSpecifications:list(list(nvpair))
                     % -Options:list(nvpair)
                     % -PositionalArguments:list(nvpair)
                 % +ParseOptions:list(nvpair)
    opt_help/2   % +OptionsSpecifications:list(list(compound))
                 % -Help:atom
  ]
).
...
## Notes and tips {#optparse-notes}
  ...
  * In the example we were mostly explicit about the types. Since the
    ...
    parsing becomes more reliable and error messages will be easier
    to interpret.
...
@author Wouter Beek
@version 0.20 (2011-04-27), 2014/04
