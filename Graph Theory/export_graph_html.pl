...
%! export_graph_html(+G_Term:compound, -G_DOM:list) is det.

export_graph_html(graph(Vs, _Ranks, Es, G_Attrs1), G_DOM):-
...
export_edge_html(Vs, edge(FromV/_FromV_Id, ToV/_ToV_Id, E_Attrs), E_DOM):-
  ...
  ;
    ...
    dcg_phrase(dcg_style(color(E_Color)), E_Style),
    % Edge name.
