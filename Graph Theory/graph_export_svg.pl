...
export_graph_svg(graph(Vs, _Ranks, Es, G_Attrs1), SVG_Root):-
  % Graph name.
  select_option(label(Name), G_Attrs1, G_Attrs2, nolabel),
  % Graph surface.
  setting(default_surface, DefaultSurface),
  select_option(surface(Surface), G_Attrs2, _G_Attrs3, DefaultSurface),
  svg_head(Surface, SVG_Head),
  
  % Unranked vertices.
  maplist(export_vertex_svg, Vs, Vs_DOM),
  
  % Unranked edges.
  maplist(export_edge_svg, Es, Es_DOM),
  append(Vs_DOM, Es_DOM, SVG_Body),
  root_element(svg, SVG_Head, SVG_Body, SVG_Root).

%! export_vertex_svg(+V_Term:compound, -V_DOM:element) is det.

export_vertex_svg(vertex(V_Id, Attrs1), V_DOM):-
  % X0 and Y0.
  select_option(coord(coord(2,[X0,Y0])), Attrs1, Attrs2),
  
  % Radius.
  setting(default_vertex_radius, DefaultR),
  select_option(radius(R), Attrs2, Attrs3, DefaultR),
  maplist(format_number(cm), [X0,Y0,R], [X0_cm,Y0_cm,R_cm]),
  
  % Name.
  select_option(label(Name), Attrs3, Attrs4, nolabel),
  
  % Color.
  setting(default_vertex_color, DefaultColor),
  select_option(color(V_Color), Attrs4, _Attrs5, DefaultColor),
